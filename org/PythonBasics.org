...
- It is customary to use the shortcut =np= for =numpy when importing it
...
- For plotting capabilities, we may use the =matplotlib= library
- It is customary to import =pyplot=, the plotting features of =matplotlib=, as
  =plt=:
#+begin_src python :session matplotlib :exports both
  print('test')
#+end_src
...
#+begin_src  python :results output :session matplotlib :exports both
...
#+begin_src python :session matplotlib :exports both :results output
  π = np.pi
...
To plot these:
1. First create a figure and axes object using the =subplots()= command
2. Then, use =plt.plot(x, y)=
   
#+begin_src python :session matplotlib :exports code
  fig, ax = plt.subplots()
  plt.plot(x, y1)

  # if using Jupyter, uncomment the following to show the plot
  # plt.show()
#+end_src

#+RESULTS:
| <matplotlib.lines.Line2D | object | at | 0x118894ad0> |

To save the figure in a file, use =plt.savefig( filename )= to save the figure
as an image
#+begin_src python :session matplotlib :exports both :results output
  import os
  figname = os.path.join('html', 'img', 'sinePlot.png')
  plt.savefig( figname )
#+end_src

#+RESULTS:

#+begin_src python :session matplotlib :exports none :results output
  orgfig( figname[5:] )
#+end_src
...
#+name: fig:defaultName
#+attr_html: :width 480px
#+caption: A simple =matplotlib= plot.
[[./img/sinePlot.png]]
...
- Add an \(x\)-label
  - To use LaTeX, we use a raw string with LaTeX syntax deliniated using the
    dollar sign, ~$~
#+begin_src python :session matplotlib :exports code :results output
  fig, ax = plt.subplots()
...
#+begin_src python :session matplotlib :exports both :results output
  import os
  figname = os.path.join('html', 'img', 'sincosPlot.png')
  plt.savefig( figname )
